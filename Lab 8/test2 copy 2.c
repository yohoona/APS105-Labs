...
int makeMove(const char board[][26], int n, char turn, int *row, int *col) {
    ...
    for (int row = 0; row < n; ++row) {
        for (int col = 0; col < n; ++col) {
            isLegal = false;
            for (int i = -1; i < 2 && isLegal == false; ++i) {
                for (int j = -1; j < 2 && isLegal == false; ++j) {
                    isLegal = checkLegalInDirection(board, n, row, col, turn, i, j);
                }
            }
            if (isLegal == true) {
                for (int i = -1; i < 2; ++i) {
                    for (int j = -1; j < 2; ++j) {
                        moveScores[row][col] *= moveScore(board, n, row, col, turn, i, j);
                    }
                }
            } 
        }
    ...
    for (int row = 0; row < n; ++row) { 
        for (int col = 0; col < n; ++col) {
            if (moveScores[row][col] > highestMoveScore) {
                moveScores[row][col] = highestMoveScore;
                moveRow = row;
                moveCol = col;
            }
        }
